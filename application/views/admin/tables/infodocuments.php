...
if ( $GLOBALS['current_user']->parent_customer_id > 0 ) {
	$subcustomer_where = ' OR ( customer = '. $GLOBALS['current_user']->parent_customer_id .' )';
}
	...
	if (get_user_role() == 'customer') {
		...
		$query  = ' SELECT provider FROM tblquotations WHERE provider != "" AND customer = ' . get_user_id() . $subcustomer_where;
		$query .= ' UNION ';
		$query .= ' SELECT provider FROM tblassignments WHERE provider != "" AND customer = ' . get_user_id() . $subcustomer_where;
