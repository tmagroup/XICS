<?php     
    ...
    $sTable        = 'tblreminders';
    $join = array('LEFT JOIN tblremindersubjects ON tblremindersubjects.id=tblreminders.remindersubject');
    
    //$where = do_action('reminders_table_sql_where', array());    
    $where = array(); 
    array_push($where, "AND tblreminders.rel_id='".$rel_id."' AND tblreminders.rel_type='".$rel_type."'");
    ...
    foreach ($rResult as $aRow) {
        ...
        . '\''.base_url("admin/reminders/editReminder").'\','
        . '\''.$aRow["remindernr"].'\','       
        ...
        . '\''.base_url("admin/reminders/deleteReminder").'\','
        . '\''.$aRow["remindernr"].'\','
