<?php     
    ...
    $sTable        = 'tblassignmentreminders';
    $join = array('LEFT JOIN tblremindersubjects ON tblremindersubjects.id=tblassignmentreminders.remindersubject');
    
    //$where = do_action('reminders_table_sql_where', array());    
    $where = array(); 
    array_push($where, "AND tblassignmentreminders.rel_id='".$rel_id."' AND tblassignmentreminders.rel_type='".$rel_type."'");
    ...
    foreach ($rResult as $aRow) {
        ...
        if(get_user_role()=='user' && isset($GLOBALS['current_user']->userrole) && $GLOBALS['current_user']->userrole!=6){
            ...
            . '\''.base_url("admin/assignmentreminders/editReminder").'\','
            . '\''.$aRow["remindernr"].'\','       
            ...
            . '\''.base_url("admin/assignmentreminders/deleteReminder").'\','
            . '\''.$aRow["remindernr"].'\','
