	...
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="id">
					<?php echo form_hidden('rel_id', $quotation['leadquotationnr']); ?>
					<?php echo form_hidden('rel_type', 'leadquotation'); ?>
				</div>
...
jQuery(document).ready(function() {
	...
	Dropzone.options.myDropzone = {
		...
		maxFilesize: (parseFloat('<?php echo file_upload_max_size();?>') / (1024 * 1024)).toFixed(0),
		...
		init: function() {
			...
			this.on('success', function(file, response) {
				...
				<?php
				/*if(isset($quotation['leadquotationnr'])){
					?>
					jQuery.ajax({url: '<?php echo base_url('admin/leadquotations/getDocuments/'.$quotation['leadquotationnr']);?>', success: function(result){
			   ...
			   success: function(data){
					...
					if(url.indexOf('deleteDocument') != -1){
						...
						if(data.response=='success'){
							...
							jQuery('#row3_old_quotationproduct_'+data.dataid).remove();
						}
						jQuery('#deleteConfirmationAjax').modal('hide');
					}
						...
						if(isset($quotation['leadquotationnr'])){
							?>
							jQuery.ajax({url: '<?php echo base_url('admin/leadquotations/getComments/'.$quotation['leadquotationnr']);?>', success: function(result){
	...
	jQuery("#customer").change( function(){
		...
		jQuery.ajax({dataType: "JSON", url: '<?php echo base_url('admin/leadquotations/getResponsibleOfCustomer/');?>'+custid, success: function(data){
			var optionhtml = '';
	...
	if (jQuery('#leadnr').val() != '') {
		jQuery('#responsible_name').val(jQuery('#leadnr').find(":selected").data("responsible"));
	}
	jQuery('#leadnr').change(function(event) {
		jQuery('#responsible, #responsible_name').val('');
		if (jQuery(this).val()) {
			jQuery('#responsible').val(jQuery(this).find(":selected").data("responsible-id"));
			jQuery('#responsible_name').val(jQuery(this).find(":selected").data("responsible"));
		}
	});
...
function addeditReminderAjax(url, id, title){
	...
	if(id>0){
		Pace.track(function(){
			Pace.restart();
			jQuery.ajax({dataType: "JSON", url: '<?php echo base_url('admin/leadquotationreminders/getReminder/');?>'+id, success: function(data){
