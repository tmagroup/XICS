				...
				<div class="page-content">
					...
					<div class="page-bar">
						<ul class="page-breadcrumb">
							...
							<li>
								<span>
									<?php
									if(isset($quotation['leadquotationnr']) && $quotation['leadquotationnr']>0){
										echo lang('page_edit_leadquotation');
									}
									else
									{
										echo lang('page_create_leadquotation');
									}
									?>
								</span>
							</li>
					...
					<div class="row">
						...
						<div class="tab-content">
											...
											<div class="form-body">
												...
												<div class="form-group">
													...
													<div class="input-group date form_date">
														<?php $dd = array('name'=>'leadquotationdate', 'class'=>'form-control', 'readonly'=>true, 'size'=>16, 'value'=> isset($quotation['leadquotationdate'])?_d($quotation['leadquotationdate']):date('d.m.Y'));
														echo form_input($dd);?>
												...
												<div class="form-group">
													...
													<?php echo form_dropdown('leadquotationstatus', $quotationstatus, isset($quotation['leadquotationstatus'])?$quotation['leadquotationstatus']:'', 'class="form-control"');?>
												</div>
									...
									<div class="portlet light bordered">
										<div class="portlet-body form">
											...
											<div class="form-body">
												...
												<div class="form-group">
													<label><?php echo lang('page_lead');?> <span class="required"> * </span></label>
													<select name="leadnr" id="leadnr" class="form-control">
														<option value=""></option>
														<?php foreach ($leads as $key => $value): ?>
															<option value="<?= $value['leadnr']?>" data-responsible-id="<?= $value['responsible']?>" data-responsible="<?= $value['responsible_name']?>"
																<?php (isset($quotation['leadnr']) && $quotation['leadnr'] == $value['leadnr']) && print 'selected'; ?>
																><?= $value['leadnr_prefix'].' ('.$value['company'].')'?></option>
														<?php endforeach ?>
													</select>
												...
												<div class="form-group">
													...
													<input type="hidden" name="responsible" id="responsible" class="form-control" value="<?php echo isset($quotation['responsible'])?$quotation['responsible']:''; ?>">
													<input type="text" id="responsible_name" class="form-control" readonly>
																		...
																		<td id="new_newratemobile_box_<?php echo $pkey;?>">
																			<?php if($formula=='A'){ ?>
																				<select name="newratemobile[<?= $pkey?>]" class="form-control newratemobile noerror" id="new_newratemobile_<?= $pkey?>" datarow="<?= $pkey?>" datatype="new">
																					<option value=""><?= lang('page_option_select')?></option>
																					<?php foreach ($mobilerates as $key => $value): ?>
																						<option value="<?= $value['ratenr']?>" <?php (isset($quotation['newratemobile']) && $quotation['newratemobile'] == $value['ratenr']) && print 'selected'; ?> data-provider="<?= $value['provider']?>"><?= $value['ratetitle']?></option>
																					<?php endforeach ?>
																				</select>
										...
										<div class="portlet-body">
											<div class="form-body">
												<div class="form-actions">
													...
													<a href="<?php echo base_url('admin/leadquotations')?>"><button type="button" class="btn default"><?php echo lang('cancel');?></button></a>
												</div>
											</div>
										</div>
									</div>
							...
							<?php echo form_open(site_url('admin/leadquotations/download_product_csv'), array('id' => 'form_import_quotation'));?>
							<?php echo form_close();?>
...
<script>
	...
	$('#submit_import_quotation').click(function(event) {
		...
		$.ajax({
			url: "<?php echo base_url('admin/leadquotations/import_product_csv');?>",
			type: 'POST',
...
function changecurrentRateMobile(){
	jQuery('.currentratemobile').change( function(){
		...
		if(formula=='A'){
			if(id==""){ id="none"; }
			jQuery.ajax({url: '<?php echo base_url('admin/leadquotations/getMobileRateValue/');?>'+id+'/'+discountlevel+'/'+formula, success: function(result){
...
function changecurrentOptionMobile(){
	jQuery('.currentoptionmobile').change( function(){
		...
		if(formula=='A'){
			jQuery.ajax({url: '<?php echo base_url('admin/leadquotations/getMobileOptionValue/');?>'+id, success: function(result){
...
function deletequotationproduct(dataid,datatype,parentid){
	...
	if(datatype=='old'){
		...
		deleteConfirmation('<?php echo base_url('admin/leadquotations/deleteQuotationProduct/');?>',dataid,'<?php echo lang('page_lb_delete_quotationproduct')?>','<?php echo lang('page_lb_delete_quotationproduct_info')?>','true',parentid);
	}
	else{
...
changeFormula();
changeHardware();
jQuery("#leadnr").select2({
	placeholder: "<?php echo lang('page_lead');?>",
	allowClear: true
});
</script>
<?php $this->load->view('admin/leadquotations/leadquotationjs',array('quotation'=>isset($quotation)?$quotation:'', 'remindersubjects'=>$remindersubjects));?>