<div class="portlet-title">
	...
	<?php
	if($GLOBALS['quotationcomment_permission']['create']){
		?>
		<div class="actions">
...
if($GLOBALS['quotationcomment_permission']['create']){
	?>
	<?php echo form_open(base_url('admin/leadquotations/addComment'),array("id"=>"addCommentAjax")); ?>
		<div id="addcommentbox">
			<?php echo form_hidden('rel_id',$quotation['leadquotationnr']);?>
			<?php echo form_hidden('rel_type','leadquotation');?>
			<?php echo form_hidden('addedto',$quotation['responsible_id']);?>
				...
				<li class="<?php if($comment['addedfrom']==get_user_id()){ echo "out"; }else{ echo "in"; }?>">
					...
					<div class="message">
						...
						<span class="body"> <?php echo $comment['description'];?>
							...
							<?php
							if($GLOBALS['quotationcomment_permission']['delete']){
								?>
								<div class="text-<?php if($comment['addedfrom']==get_user_id()){ echo "right"; }else{ echo "left"; }?>">
									<a href="javascript:void(0);" class="text-danger" onclick="deleteConfirmation('<?php echo base_url("admin/leadquotations/deleteComment");?>','<?php echo $comment['id'];?>','<?php echo lang("page_lb_delete_quotationcomment");?>','<?php echo lang("page_lb_delete_quotationcomment_info");?>','true','<?php echo $comment['rel_id'];?>');"><i class="fa fa fa-times"></i></a>
								</div>
								<?php
							}
							?>
