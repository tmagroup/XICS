...
class Email_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->user_email_templates  = get_user_email_templates_slugs();
    }
    ...
    public function send_simple_email($email, $subject, $message)
    {
        $cnf = array(
            ...
            'from_name' => get_option('company_name'),
            'email' => $email,
            'subject' => $subject,
            'message' => $message
        );
     ...
     * @param  mixed $cc
     * @param  number $todonr for storing message and subject
     * @return boolean
     */
    public function send_email_template($template_slug, $email, $merge_fields, $ticketid = '', $cc = '', $todonr = '')
    {
        $email = do_action('send_email_template_to', $email);

        $template                     = get_email_template_for_sending($template_slug, $email);
        $user_email_templates_slugs  = get_user_email_templates_slugs();

        $inactive_user_table_check = '';

        /**
         * Dont send email templates for non active users
         * Do checking here
         */
        if (in_array($template_slug, $user_email_templates_slugs)) {
            $inactive_user_table_check = 'tblusers';
        }
        ...
        if ($template->active == 0 || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $this->db->where('language', 'german');
            $this->db->where('slug', $template->slug);
            $tmpTemplate = $this->db->get('tblemailtemplates')->row();

            if (!$tmpTemplate) {
                logActivity('Failed to send email template [<a href="'.base_url('admin/emails/email_template/'.$tmpTemplate->emailtemplateid).'">'.$template->name.'</a>] [Reason: Email template is disabled.]');
            }
        ...
        */
        ...
        // echo '<pre>';
        // $this->email->to('connectusdemo12@gmail.com');
        // print_r($this->email);
        // print_r($template);
        // $this->email->send();
        // return true;
        ...
        if ($this->email->send()) {
            logActivity('Email Send To [Email: ' . $email . ', Template: ' . $template->name .', template_slug: '. $template_slug . ']');

            if ($todonr != '') {
                $this->db->where('todonr', $todonr);
                $this->db->update('tbltodos', array('todotitle' => $template->subject, 'tododesc' => htmlentities($template->message)));
            }
