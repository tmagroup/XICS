...
class Quotation_model extends CI_Model
{
    ...
    public function get($id='', $field='', $join=array(), $where="", $groupby="")
    {
        ...
        if (is_numeric($id)) {
            ...
            $quotation = $this->db->get($this->table)->row();
            if ($quotation) {
                $quotation->attachments = $this->get_quotation_attachments($id);
            }
            return $quotation;
        }
    ...
    public function add($data)
    {
        ...
        $data['quotationdate'] = to_sql_date($data['quotationdate'], false);
        ...
        if($id>0){
            ...
            //Add ID Prefix
            $dataId = array();
            $dataId['quotationnr_prefix'] = idprefix('quotation',$id);
            ...
            if(isset($data['mobilenr'])){
                foreach($data['mobilenr'] as $fk=>$fd){
                    ...
                    $dataproduct = array('quotationnr'=>$id,'mobilenr'=>$fd,
                    ...
                    if(isset($data['quotationproductid'][$fk]) && $data['quotationproductid'][$fk]>0){
                        //if(trim($fd)!=""){
                            ...
                            $this->Quotationproduct_model->update($dataproduct, $data['quotationproductid'][$fk]);
                        /*}else{
                            $this->Quotationproduct_model->delete($data['quotationproductid'][$fk]);
                        }*/
                    }else{
                        //if(trim($fd)!=""){
                            $dataproduct['formula'] = $data['new_formula_'.$fk];
                            $this->Quotationproduct_model->add($dataproduct);
                        //}
                    }
                }
            }
            ...
            //Get Quotationnr
            $rowfield = $this->get($id,'quotationnr_prefix');
            //Log Activity
            logActivity('New Quotation Added [ID: ' . $id . ', ' . $rowfield->quotationnr_prefix . ']');
        }
    ...
    public function update($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $rowfield = $this->get($id,'quotationnr_prefix');
            //Log Activity
            logActivity('Quotation Updated [ID: ' . $id . ', ' . $rowfield->quotationnr_prefix . ']');
        }
    ...
    public function delete($id)
    {
        ...
        //Log Activity
        logActivity('Quotation Deleted [ID: ' . $id . ', ' . $rowfield->quotationnr_prefix . ']');

        //Delete Quotation Products
        $quotationproducts = $this->Quotationproduct_model->get('', 'id', array(), " quotationnr='".$id."' ");
        if(isset($quotationproducts) && count($quotationproducts)>0){
            foreach($quotationproducts as $quotationproduct){
               $this->Quotationproduct_model->delete($quotationproduct['id']);
            }
        }
        ...
        //Delete Reminder
        $reminders = $this->Quotationreminder_model->get('', 'remindernr', array(), " rel_id='".$id."' AND rel_type='quotation' ");
        if(isset($reminders) && count($reminders)>0){
            foreach($reminders as $reminder){
               $this->Quotationreminder_model->delete($reminder['remindernr']);
            }
        }
    ...
    public function add_attachment_to_database($rel_id, $attachment, $external = false, $form_activity = false)
    {
        ...
        if($insert_id>0){
            $quotation = $this->get($rel_id,'quotationnr_prefix');
            logActivity('Quotation Attachment Added [QuotationID: ' . $rel_id . ', '.$quotation->quotationnr_prefix.']');

            //History
            $Action_data = array('actionname'=>'quotation', 'actionid'=>$rel_id, 'actionsubid'=>$insert_id, 'actiontitle'=>'quotation_document_added');
    ...
    public function delete_quotation_attachment($id)
    {
        ...
        $quotation = $this->get($attachment->rel_id,'quotationnr_prefix');
        $deleted    = false;
        ...
        if ($attachment) {
            ...
            if ($this->db->affected_rows() > 0) {
                $deleted = true;
                logActivity('Quotation Attachment Deleted [QuotationID: ' . $attachment->rel_id . ', '.$quotation->quotationnr_prefix.']');

                //History
                $Action_data = array('actionname'=>'quotation', 'actionid'=>$attachment->rel_id, 'actionsubid'=>$attachment->id, 'actiontitle'=>'quotation_document_deleted');
                do_action_history($Action_data);
            }
    ...
    public function update_quotation_attachmentcategory($rel_id, $categoryid){
        ...
        $this->db->update('tblfiles', $data, array('rel_id' => $rel_id, 'rel_type'=>'quotation'));
        if ($this->db->affected_rows() > 0) {
            $quotation = $this->get($rel_id,'quotationnr_prefix');
            logActivity('Quotation Attachment Updated [QuotationID: ' . $rel_id . ', '.$quotation->quotationnr_prefix.']');
            return 1;
        }
    }
    ...
    public function sendReminder($reminderid='', $submit_type=''){
        if($submit_type=='single'){
            //Reminder Quotation
            $data = (array) $this->Quotationreminder_model->get($reminderid,'tblquotationreminders.remindernr, tblremindersubjects.name as remindersubject, '
                . 'tblsalutations.name as salutation, '
                . 'responsible.email, '
                . 'responsible.name, '
                . 'responsible.surname, '
                . 'tblquotations.quotationnr, '
                . 'tblquotations.quotationnr_prefix, '
                . 'tblquotations.company, '
                ...
                'tblquotations'=>'tblquotations.quotationnr=tblquotationreminders.rel_id',
            ...
            $data = (array) $this->Quotationreminder_model->get($reminderid,'tblquotationreminders.remindernr, tblremindersubjects.name as remindersubject, '
                ...
                'tblsalutations'=>'tblsalutations.salutationid=responsible.salutation',
                    ...
                    'tblcustomers'=>'tblcustomers.customernr=tblquotations.customer',
                ),
    ...
    function sendMail($data){
        $data['linktoreminder'] = '<a href="'.base_url('admin/quotations/detail/'. $data['quotationnr']).'" target="_blank">'.lang('click_here').'</a>';
        $merge_fields = array();
        ...
        if ($sent) {
            ...
            $this->Quotationreminder_model->update($post,$data['remindernr']);
    ...
    public function sendReminder_quotation($quotationnr='', $submit_type=''){

        if($submit_type=='single'){
            //Reminder Quotation
            $data = (array) $this->Quotation_model->get($quotationnr,' '
                . 'tblsalutations.name as salutation, '
                . 'responsible.email, '
                . 'responsible.name, '
                . 'responsible.surname, '
                ...
                . 'tblquotations.quotationnr_prefix, '
                . 'tblquotations.company,'
                . 'tblquotations.quotationdate,'
                . 'tblquotations.responsible ',

                array('tblusers as responsible'=>'responsible.userid=tblquotations.responsible',
                'tblsalutations'=>'tblsalutations.salutationid=responsible.salutation'
                ),

                "tblquotations.quotationstatus='1'"
            );
        }
        else{
            //Reminder Quotation
        ...
        if(isset($data['quotationnr'])){
            ...
            /*******************************************************************/
            $rowSend = $this->Quotationreminderopen_model->get('', '', array()," quotationnr='".$data['quotationnr']."' ","sendno", "1");
    ...
    function sendMail_quotation($data,$no){
        $data['linktoquotation'] = '<a href="'.base_url('admin/quotations/detail/'. $data['quotationnr']).'" target="_blank">'.lang('click_here').'</a>';
        $merge_fields = array();
        ...
        if ($sent) {
            $post = array('quotationnr'=>$data['quotationnr'],'responsible'=>$data['responsible'],'sendno'=>$no);
            $this->Quotationreminderopen_model->add($post);
            do_action('quotationreminderopen_sent', $data['quotationnr']);
            return 1;
        }
        else{
            return 0;
