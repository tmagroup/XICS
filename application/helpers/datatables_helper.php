 ...
 * @return array
 */
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = array(), $where = array(), $additionalSelect = array(), $sGroupBy = '', $customOrderBy = '')
{
    $CI =& get_instance();
    ...
    if ($CI->input->post('order')) {
        $sOrder = "ORDER BY ";
        $sOrder .= ($customOrderBy != '') ? $customOrderBy.', ' : '';
        ...
        $sOrder = ($customOrderBy != '') ? "ORDER BY ".$customOrderBy : '';
    }
    /*
     * Filtering
    ...
    if ((isset($__post['search'])) && $__post['search']['value'] != "") {
        ...
        $sWhere      = "WHERE (";
        for ($i = 0; $i < count($aColumns); $i++) {
            if ((isset($__post['columns'][$i])) && isset($__post['columns'][$i]['searchable']) && $__post['columns'][$i]['searchable'] == "true") {
	...
	//Custom Order Column
	$sOrder = str_replace("_prefix","",$sOrder);
...
function my_data_tables_init($aColumns, $sIndexColumn, $sTable, $join = array(), $where = array(), $additionalSelect = array(), $sGroupBy = '')
{
    ...
    $rResult = $CI->legacy_db->query($sQuery)->result_array();
    ...
    SELECT FOUND_ROWS()
    ";
    $_query         = $CI->legacy_db->query($sQuery)->result_array();
    $iFilteredTotal = $_query[0]['FOUND_ROWS()'];
    if (_startsWith($where, 'AND')) {
        $where = 'WHERE ' . substr($where, 3);
    }
...
function sharetask_render_datatable($headings = array(), $class = '', $additional_classes = array(''), $table_attributes = array())
{
    $_additional_classes = '';
    $_table_attributes   = ' ';
    if (count($additional_classes) > 0) {
    ...
    $table = '<div class="' . $IEfix . '"><table' . $_table_attributes . 'class="dt-table-loading table table-sheretask-dt' . $_additional_classes . '">';
    $table .= '<thead>';
    $table .= '<tr>';
    foreach ($headings as $heading) {
        if(!is_array($heading)){
