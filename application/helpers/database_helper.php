...
{
    $CI =& get_instance();
    $user =  $CI->object_cache->get('user-name-data-'.$userid);

    if (!$user) {
        $CI->db->where('userid', $userid);
        $user = $CI->db->select('name')->from('tblusers')->get()->row();
        $CI->object_cache->add('user-name-data-'.$userid, $user);
    }

    return $user ? $user->name : '';
}

function get_staff_emailaddress($userid = '')
{
    $tmpStaffUserId = get_staff_user_id();
    if ($userid == "" || $userid == $tmpStaffUserId) {
        if (isset($GLOBALS['current_user'])) {
    ...
    $staff =  $CI->object_cache->get('staff-email-data-'.$userid);

    if (!$staff) {
        $CI->db->where('staffid', $userid);
        $staff = $CI->db->select('email')->from('tblstaff')->get()->row();
        $CI->object_cache->add('staff-email-data-'.$userid, $staff);
    }
 ...
 * @return mixed  Total rows
 */
function total_sharing_rows($table, $where = array())
{
    $CI =& get_instance();
    $db = $CI->load->database('sharing_db', true);
...
function my_total_rows($table, $where = array())
{
    $CI =& get_instance();
    $CI->legacy_db = $CI->load->database('sharing_db', true);
    if (is_array($where)) {
        if (sizeof($where) > 0) {
            $CI->legacy_db->where($where);
        }
    } elseif (strlen($where) > 0) {
        $CI->legacy_db->where($where);
    }

    return $CI->legacy_db->count_all_results($table);
}
/**
...
/**
 * Get id prefix
 * @param  mixed prefix name and id
 * @return mixed original con cat prefix
 */
function idprefix($name, $id){
    $CI =& get_instance();
    $CI->db->select('prefix');
    $CI->db->where('name', trim($name));
    $prefix = $CI->db->get('tblprefixes')->row('prefix');
	$prefix = str_replace("%ID",$id,$prefix);
...
/* Download Sample CSV
*/
function downloadsamplecsv($data, $separator = ','){
   ...
   //force_download('ratesmobile_sample_import_file.csv', $data);//Force Download
   echo $_data;
   exit;
}
